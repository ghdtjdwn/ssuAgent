...
있으므로 모델과 사용자에게는 고정된 일반 오류만 전달하고 로그에는 예외 type만 남긴다. 이
경계를 도서관 custom loop와 supervisor에도 동일하게 적용한다.
...
개인 학사 의도로 분류되고 provider preflight가 CONNECTED일 때만 session-bound private tool을
만든다. 상태 계약 누락, malformed 응답, timeout, provider health ERROR는 fail-safe로 고정된
상태 확인 실패 안내를 반환한다.
...
- 도구 예외 원문은 다음 모델 turn·checkpoint·SSE에 남기지 않는다.
- `AUTH_REQUIRED`, `NO_SESSION`, `INVALID_SESSION`, `SESSION_MISMATCH`는 구조화된 status로
  차단하고, 정상 데이터 안의 같은 문자열은 차단하지 않는다.
- preflight의 malformed/expired/error/timeout/미지원 계약을 fail-safe로 검증한다.
